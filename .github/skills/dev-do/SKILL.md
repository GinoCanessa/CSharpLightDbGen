...
4. **Final verification.** When the loop exits because all phases are
   ...
   default, `dotnet test db-gen.slnx`) once more end-to-end.
...
- **Honor repo conventions and stored memories** — explicit C# types,
  `[]` for empty collections, `dotnet build db-gen.slnx` and
  `dotnet test db-gen.slnx` as the canonical build/test commands, and any
