...
- **Consistency** — does this change follow the patterns already
  ...
  `dotnet build db-gen.slnx` as the canonical build).
...
1. **Target** *(required)* — where to write the analysis. One of:
   - A **full path** (absolute or repo-relative) to a `.md` file. Used
     ...
     `C:\ai\git\CSharpLightDbGen\scratch\0501-04\analysis.md`.
...
3. **Pre-flight.**
   ...
   - Confirm the build/test commands referenced by the repo (e.g.,
     `dotnet build db-gen.slnx`, `dotnet test db-gen.slnx`)
...
- {Specific commands. E.g.,
  `dotnet test db-gen.slnx --filter FullyQualifiedName~Foo`}
...
- **Honor repo conventions and stored memories.** Use them as the
  ...
  empty collections, `dotnet build db-gen.slnx` and
  `dotnet test db-gen.slnx` as the canonical build/test
